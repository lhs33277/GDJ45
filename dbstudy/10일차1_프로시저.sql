/*
    ...
    1. 여러 개의 쿼리를 한 번에 처리할 수 있다.
    2. 동시에 진행해야 하는 쿼리문들을 한 번에 수행할 수 있다.
...
IS
BEGIN
    DBMS_OUTPUT.PUT_LINE('Helli Procedure');
END PROC1;
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(var_employee_id);
END PROC2;
...
DECLARE
    var_last_name EMPLOYEES.LAST_NAME%TYPE;
BEGIN
    PROC4(var_last_name);   -- 프로그래밍 내부에서는 EXCUTE가 없어야 프로시저를 호출할 수 있다.
    DBMS_OUTPUT.PUT_LINE(var_last_name);
END;
...
BEGIN
    SELECT SALARY
     ...
     WHERE EMPLOYEE_ID = var_employee_id;
EXCEPTION
    WHEN NO_DATA_FOUND THEN -- WHEN OTHERS THEN으로 처리할 수 있다.
        var_salary := 0;
END PROC6;
...
ALTER TABLE PRODUCTS ADD CONSTRAINT PRODUCT_PK PRIMARY KEY(PROD_CODE);
...
ALTER TABLE CUSTOMERS ADD CONSTRAINT CUSTOMERS_PK PRIMARY KEY(CUST_NO);
...
ALTER TABLE BUYS ADD CONSTRAINT BUYS_CUSTOMERS_FK FOREIGN KEY(CUST_NO) REFERENCES CUSTOMERS(CUST_NO);
ALTER TABLE BUYS ADD CONSTRAINT BUYS_PRODUCTS_FK FOREIGN KEY(PROD_CODE) REFERENCES PRODUCTS(PROD_CODE);
