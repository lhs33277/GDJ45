...
/*
    ...
    3. SELECT절에서 사용되면 '스칼라 서브쿼리', FROM절에서 사용되면 '인라인뷰', WHERE절에서 사용되면 '서브쿼리'라고 한다.
    4. 종류
        1) 단일 행(SINGLE ROW) 서브쿼리 : 서브쿼리 결과가 1개, 단일 행 연산(=, !=, >, <, >=, <=) 사용
        2) 다중 행(MULTI ROW) 서브쿼리  : 서브쿼리 결과가 여러개, 다중 행 연산(IN, ANY, ALL 등)
*/
...
--    메인쿼리 : SELECT 칼럼 FROM DEPARTMENT WHERE 지역 = (부서번호(DEPT_NO)가 1인 부서의 지역)
...
-- 6. 부서번호(DEPART)가 2인 부서에 존재하는 사원들의 직급과 일치하는 직급을 가진 사원을 조회하시오.
--    서브쿼리 : 부서번호(DEPART)가 2인 부서에 존재하는 사원들의 직급 (다중 행 : 부서번호(DEPART)칼럼은 PK도 아니고
--                                                                               UNIQUE도 아니므로 서브쿼리 결과가 여러개)
