...
-- 2. 그룹화는 대부분 그룹함수와 함께 사용된다. 그냥 사용하면 DISTINCT와 다를게 없다.
...
--    해당 그룹의 개수가 2개 이상인 부서만 조회하시오.
--    그룹별 개수 조회 이후에 지정할 수 있는 조건이므로 HAVING절에서 처리해야 한다.
SELECT SUBSTR(DEPARTMENT_NAME, 1, 2), COUNT(*)
  FROM DEPARTMENTS
 GROUP BY  SUBSTR(DEPARTMENT_NAME, 1, 2)
HAVING COUNT(*) >= 2;
...
SELECT DEPARTMENT_ID AS 부서번호, COUNT(*) AS 사원수
 ...
 GROUP BY DEPARTMENT_ID
HAVING COUNT(*) >= 5
 ORDER BY DEPARTMENT_ID DESC;   -- ORDER BY 부서번호 : ORDER BY절은 SELECT절 이후에 처리되므로 SELECT절에서 지정한 별명을 사용할 수 있다.
