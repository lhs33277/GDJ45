...
<script src="resources/js/jquery-3.6.0.js"></script> <!-- 외부 js 파일 포함하기 -->
<script>
	
	// 페이지 로드 이벤트 (호이스팅. 자바스크립트는 선언부를 먼저 읽어보고 그 다음 다시 읽어본다.)
	$(document).ready(function(){
		$('#btn1').on('click', function(){ fnAjax1(); })  // btn1을 클릭하면 fnAjax1() 함수를 실행하시오.
		$('#btn2').on('click', function(){ fnAjax2(); })
		$('#btn3').on('click', fnAjax3)					  // 이렇게 적어도 되는데 잘 안씀
	...
	function fnAjax1(){
		...
		$('#result').empty();	// 결과가 덮어씌일 수 있으니 초기화
		
		$.ajax({
			
			/* 요청에 관한 프로퍼티 : url, type, data(파라미터), contentType */
			...
			dataType: 'text',		// 응답 데이터 타입이 텍스트 produces="text/plain; charset=UTF-8" 
			success: function(res){	// 응답 데이터는 res에 전달
				$('#result').text(res);	// <div id="result"></div>
			},
	...
	function fnAjax4(){
		...
		$.ajax({
			...
			// JSON 데이터를 주소 창에 붙여서 보낼 수 없다. (get 방식이 아니다.)
