...
public class EmpDAO {
	...
	// MybatisConfig 인스턴스					싱글톤이라 인스턴스 못만듬
	...
	public List<Emp> selectEmpList() {
		...
		// 2) 쿼리문 실행하기
		//	  (1) 매퍼 emp.xml
		//		  <mapper namespace="mybatis.mapper.emp">     mapspace : mapper가 존재하는 전체경로
		//	  (2) 쿼리문
		//		  <select id="selectEmpList">
		//	  (3) 메소드
		//		  - selectOne()  : 반환 결과가 1개
		//	      - selectList() : 반환 결과가 여러 개
		List<Emp> list = ss.selectList("mybatis.mapper.emp.selectEmpList");	// 매퍼 맵스페이스랑 쿼리문아이디를 합친다.
	...
	public int insertEmp(Emp emp) {
		...
		//	  openSession(flase) : commit이 필요한 insert/update/delete
