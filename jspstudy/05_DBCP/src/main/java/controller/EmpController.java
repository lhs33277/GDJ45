...
public class EmpController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String command = requestURI.substring(contextPath.length() + 1);	// command엔 insert.do가 담김
		...
		// Service 실행
		if(service != null) {
			af = service.execute(request, response);	// 여기로 이름 부서 파라미터 전달됨 그리고 addservice 실행하면서 다시넘김
		}
